...
import com.hausontech.hrs.daoImpl.dataPorcessing.mapper.AllocScheduleInstanceMapper1;
...
public class DataProcessDaoImpl extends BaseDaoImpl implements IDataProcessDao {
	...
	public List<Map<String, Object>> getRequestInstanceList(RequestInstanceBean requestBean) throws SQLException {
		String strCondition = buildCondition(requestBean);//添加where条件
		StringBuffer sql = new StringBuffer();
		...
		if (condition != null) {
			...
			if (condition instanceof AllocRequestInstanceRecord) {
				...
				if(null != searchBean && !searchBean.isEmpty()) {
					...
					String RuleType = searchBean.getRuleType();
					if(null != RuleType && !RuleType.isEmpty()){
						sb.append(" and RULE_TYPE = '"+ RuleType + "' ");	
					}
			...
			if (condition instanceof AllocScheduleInstanceBean) {
				AllocScheduleInstanceBean searchBean = (AllocScheduleInstanceBean)condition;
	...
	public int updateAllocSchedule(AllocScheduleInstanceBean reqBean) throws SQLException {
		...
		try {
			StringBuffer sql = new StringBuffer();
			sql.append("UPDATE HAE_ALLOC_SCHEDULE SET");
	...
	@Override
	public List<Map<String, Object>> getAllocScheduleInstance(AllocScheduleInstanceBean scheduleBean)
			throws SQLException {
		String strCondition = buildCondition(scheduleBean);
		StringBuffer sql = new StringBuffer();
		List<Map<String, Object>> list = null;
		try {
			sql.append("SELECT ");
			sql.append(" sch.INSTANCE_ID,sch.SCHEDULE_ID,sch.TYPE_PROCESS,sch.VALUE_PROCESS,sch.CONCURRENT,ins.RULE_TYPE,ins.DESCRIPTION ");
			if (scheduleBean.getRowStartIndex() != 0 && scheduleBean.getRowCount() != 0) {
				sql.append(", ROW_NUMBER() OVER (ORDER BY SCHEDULE_ID DESC) AS row_number");
			}
			sql.append(" FROM HAE_ALLOC_SCHEDULE sch,HAE_ALLOC_INSTANCE ins where sch.INSTANCE_ID=ins.INSTANCE_ID");
			if (null != strCondition && strCondition.length() > 0) {
				sql.append( strCondition);
			}
			sql.append(" ORDER BY SCHEDULE_ID DESC");
			if (scheduleBean.getRowStartIndex() != 0 && scheduleBean.getRowCount() != 0) {
				sql = this.pageSql(sql, scheduleBean.getRowStartIndex(), scheduleBean.getRowCount());
			}
			list = namedjdbcTemplate.getJdbcOperations().queryForList(sql.toString());
	...
	public int getAllocScheduleInstanceCount(AllocScheduleInstanceBean scheduleBean) throws SQLException {
		try {
			...
			if (null != strCondition && strCondition.length() > 0) {
				sql.append("(select * from HAE_ALLOC_SCHEDULE schedule,HAE_ALLOC_INSTANCE instance where  INSTANCE.INSTANCE_ID=schedule.INSTANCE_ID " + strCondition.replace("RULE_TYPE", "INSTANCE.RULE_TYPE")+")");
			}else{
				sql.append(" HAE_ALLOC_SCHEDULE");
			}
			return (int)namedjdbcTemplate.getJdbcOperations().queryForObject(sql.toString(), int.class);
		} catch (Exception e) {
			throw new SQLException(e);
	...
	public int saveAllocScheduleInstance(AllocScheduleInstanceBean scheduleBean) throws SQLException {
		...
		try {
			//get primary key from sequence
			primaryKey = this.getAutoGeneratedSchedulePrimaryKey("HAE_ALLOC_SCHEDULE_S");
			scheduleBean.setScheduleId(primaryKey);
			String sql = "INSERT INTO HAE_ALLOC_SCHEDULE(INSTANCE_ID,SCHEDULE_ID, TYPE_PROCESS , VALUE_PROCESS, CONCURRENT " + this.AUDIT_QUERY_STRING + ")" +
			 		      " VALUES(:instanceId, :scheduleId, :typeProcess, :valueProcess, :concurrent, " +
		                          ":creationDate, :createdBy, :lastUpdateDate, :lastUpdatedBy)";				
			SqlParameterSource namedParameters = new BeanPropertySqlParameterSource(scheduleBean);
			//GeneratedKeyHolder generatedKeyHolder = new GeneratedKeyHolder();
		    this.namedjdbcTemplate.update(sql, namedParameters);//, generatedKeyHolder, new String[]{"DIMENSION_ID"});		    
		} catch (Exception e) {
			throw new SQLException(e);
	...
	public int deleteScheduleRecord(AllocScheduleInstanceBean scheduleBean) throws SQLException {
		...
		try {
			StringBuffer sql = new StringBuffer();
			sql.append("DELETE HAE_ALLOC_SCHEDULE");
			sql.append(" WHERE SCHEDULE_ID = :scheduleId");
	...
	public AllocScheduleInstanceBean getScheduleById(String scheduleId) throws SQLException {
		// TODO Auto-generated method stub
			...
			try {
				sql.append("SELECT ");
				sql.append("SCHEDULE_ID, INSTANCE_ID, TYPE_PROCESS, VALUE_PROCESS, CONCURRENT ");
				sql.append(" FROM HAE_ALLOC_SCHEDULE ");
				sql.append(" WHERE SCHEDULE_ID=" + scheduleId);
				result =(AllocScheduleInstanceBean) namedjdbcTemplate.getJdbcOperations().queryForObject(sql.toString(), new AllocScheduleInstanceMapper1());
			} catch (Exception e) {
				throw new SQLException(e);
			}
		return result;
	...
	public int deleteScheduleRecordForJob(AllocRequestInstanceRecord jobBean) throws SQLException {
		...
		try {
			...
			sql.append("DELETE HAE_ALLOC_SCHEDULE");
			sql.append(" WHERE INSTANCE_ID = :instanceId");
			SqlParameterSource namedParameters = new BeanPropertySqlParameterSource(jobBean);
			// GeneratedKeyHolder generatedKeyHolder = new GeneratedKeyHolder();
			delNum = this.namedjdbcTemplate.update(sql.toString(), namedParameters);// ,																	// String[]{"DIMENSION_ID"});
		} catch (Exception e) {
			throw new SQLException(e);
