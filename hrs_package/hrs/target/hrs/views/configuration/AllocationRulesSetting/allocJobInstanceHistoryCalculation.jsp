...
<script type="text/javascript">  
		...
		var tabTile="历史记录";
		var instanceId=${requestInstance.instanceId};
	    ...
	    $(function(){
	    	$('#tableview').edatagrid({
				...
				columns:[[
					{field:'RULE_ID', title:'规则',align: 'center', 
					...
					width:50	}, 
					{field:'GROUP_HEADER_ID', title:'所属规则组',align: 'center', 
						formatter:function(value, row, index){
							for(var i=0; i < groupList.length; i++){
							//alert(groupList[i].RULE_ID+"-----"+groupList[i].RULE_NAME);
							    if (groupList[i].RULE_ID == value) 
								    return groupList[i].RULE_NAME;
							}
							if(value==0){
					        	value='';
					        }
					...
					width:40}, 	
					{field:'PERIOD', title:'会计期间',align: 'center', width:30}, 	

					{field:'STATUS', title:'状态',align: 'center', width:50}, 	
					{field:'INFORMATION',title: "信息",halign: 'center', width:80, 
					formatter:function(value,row,index) {						
							if (row.STATUS=='W101'){
								var s = '警告:分摊源为空 ';
								return s;
					...
					{field:'START_TIME', title:'开始时间',align: 'center', width:80}, 	
					{field:'END_TIME', title:'结束时间',align: 'center', width:80}, 	
					{field:'ROLLBACK_TIME', title:'回滚时间',align: 'center', width:80},
					{field:'ELAPSED_TIME', title:'花费时间',align: 'center', width:50,formatter:function(value, row, index){
							if(value==0){
					        	value='';
					        }
							return value;
