...
<head>
	...
	/*	    $.ajax({    
		   ...
		   $.ajax({    
		    ...
		    //txnProcessTypes="{value:'CNY',key:'CNY'}";
		  	var ledgerList='{}';
		   $.ajax({    
		...
		//		pagination:true,
				...
				columns:[[
				   {field:'typeCode',title:'事务处理类型',width:100,
					...
					{field:'ledgerId', title:'账套',width:80,
					...
					{field:'periodName',title:'会计期间',width:80,
						formatter:function(value, row, index){
					...
					{field:'transactionDate', title:'事务导入时间', width:80,align:'center'},
					{field:'description', title:'事务说明',halign: 'center', width:380, editor:'text'},
					{field:'auditFlag', title:'审核状态', width:60,
						formatter:function(value){
							if (value == 'Y') {
								return '已审批';
							} else {
					...
					{field:'transferFlag', title:'过账状态', width:60,align:'center',
						formatter:function(value){
							if (value == 'Y') {
								return '已过账';
							} else {
					...
					{field:'creationDate', title:'创建时间', width:100, hidden:true},
					{field:'createdBy', title:'创建人', width:100, hidden:true},
					{field:'lastUpdateDate', title:'最后更新时间', width:100, hidden:true},
					{field:'lastUpdatedBy', title:'最后更新人', width:100, hidden:true},
					{field:'txnHeaderId', title:'事务处理头ID', width:100, hidden:true}
				]],
		...
		if (selrow == null) {
			...
			$.messager.confirm('Confirm', '你是否要删除当前记录?', function(r) {
				if (r) {
					$.post('/hrs/txnProcess.do?delTxnProcHeader', {
						"headerId" : selrow.txnHeaderId
					},
						function(data) {
							if (data != null) {
...
<body>
     <form:form id="requestExceForm" commandName="executCondition" action="" method="post">        
        <div id="tb" style="padding:3px">
                <span>事务处理类型:</span>
	            <form:select path="tnxTypeCode" id="tnxTypeCode" name="tnxTypeCode" items="${executCondition.tnxTypelist}" itemLabel="name"  itemValue="code" class="easyui-combobox" style="width:230"></form:select>
	              &nbsp;&nbsp;&nbsp;	
	            <span>账套:</span>
	            <form:select path="ledgerCode" id="ledgerCode" name="ledgerCode"  items="${executCondition.ledgerList}" itemLabel="lederName" itemValue="lederCode" class="easyui-combobox" style="width:230"></form:select>
	             &nbsp;&nbsp;&nbsp;
	             <span>会计区间:</span>   
	              <form:select path="periodCode" id="periodCode" name="periodCode" items="${executCondition.periodList}" itemLabel="periodName" itemValue="periodName" class="easyui-combobox" style="width:200"></form:select>        
	             &nbsp;&nbsp;&nbsp;
