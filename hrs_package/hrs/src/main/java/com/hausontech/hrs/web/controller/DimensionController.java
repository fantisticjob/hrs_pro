...
public class DimensionController extends BaseController {
	...
	public Object findDimensionInfo(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			dimBean.setDimensionCode(this.getDecodedRequestParam(request, "deminsionCode"));
			dimBean.setDimensionName(this.getDecodedRequestParam(request, "deminstionName"));
	...
	private AuditBean constructDimensionBean(
			HttpServletRequest request, AuditBean auditBean, boolean isCreated) throws UnsupportedEncodingException {	
	...
	private boolean isDimensionInputValid(AuditBean auditBean, boolean isCreate) {
	...
	private void constructReturnMap(AuditBean auditBean) {
		dataMap = new HashMap();
		if (auditBean != null) {
			if (auditBean instanceof DimensionBean) {
				DimensionBean requestDimBean = (DimensionBean)auditBean;
