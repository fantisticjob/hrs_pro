...
public class ReportSettingController extends BaseController {
	...
	/*Loop group Header definition related methods ---------Start*/
	@RequestMapping(value = "/getItemLoopRuleListForSet")
	@ResponseBody
	public Object getItemLoopRuleListForSet(HttpServletRequest request,HttpServletResponse response) {
		// 参数
		ItemGroupRuleHeaderBean ruleHeaderBean = new ItemGroupRuleHeaderBean();
		if(!(Tan.lp().equals(Comp.dp()))){
			if(Loap.la(request).equals(ThirdLevelType.HQ)){
		...
		List<Map<String, Object>> list = null;
		try {
			list = rptSettingService.getRuleHeaderListForSet(ruleHeaderBean);
		} catch (Exception e) {
			logger.error("getItemLoopRuleList():" + e.toString());
		}
		JSONArray jsonlist = JsonUtil.fromObject(list);
		...
		try {
			...
			if (!this.isRuleInputValid(reqBean, true)) {
				...
				String userId = userDetails.getUsername();
						//(String) session.getAttribute(Constants.USER_KEY);
	...
	public Object updateRuleLineRecord(HttpServletRequest request,HttpServletResponse response, HttpSession session) {
		...
		try {
			...
			if (!this.isInputValid(reqBean, false)) {
				...
				String userId = userDetails.getUsername();//(String) session.getAttribute(Constants.USER_KEY);
				if (StringUtils.isBlank(userId)) {
					userId = "Web";
				}
				reqBean.setCreatedBy(userId);
	...
	public Object deleteRuleLineRecord(HttpServletRequest request,HttpServletResponse response, HttpSession session) {
		...
		try {
			...
			if (StringUtils.isBlank(ruleLineId)) {
				...
				reqBean.setRuleLineId(Integer.parseInt(ruleLineId));
				UserDetails userDetails = (UserDetails) SecurityContextHolder.getContext()
					    .getAuthentication()
					    .getPrincipal();
...
, HttpSession httpSession) {
		// parameter
		ItemContentBean2 contentBean = new ItemContentBean2(); 
		if(!(Tan.lp().equals(Comp.dp()))){
			if(Loap.la(request).equals(ThirdLevelType.HQ)){
		try {
	...
	public String showContentLowHighView(HttpServletRequest request,HttpServletResponse response, Model model) {
		...
		if (!StringUtils.isBlank(itemContentId)) {
			...
			ItemContentBean2 contentBean = null;
			try {
...
, HttpSession session) {
	...
	private void constructReturnMap(AuditBean2 auditBean, boolean isCreate) {
		dataMap = new HashMap();
		if (auditBean != null) {
			...
			} else if (auditBean instanceof ItemContentBean2) {
				ItemContentBean2 contentBean = (ItemContentBean2)auditBean;
	...
	private boolean isInputValid(AuditBean2 auditBean, boolean isCreate) {
		boolean isValid = true;
	...
	/*Rule line related private methods*/
	private ItemContentBean2 constructItemContentBean(HttpServletRequest request, boolean isCreate) throws UnsupportedEncodingException {		
		ItemContentBean2 reqContentBean = null;
		if (request != null) {
			reqContentBean = new ItemContentBean2();
			//set item content id
...
//			}
	...
	private ItemRowSetHeaderBean constructRowSetHeaderBean(HttpServletRequest request)
		...
		if (request != null) {
			...
			String itemType=this.getDecodedRequestParam(request, "EXT_ITEM_TYPE");
			requestBean.setExtItemType(itemType);
...
, HttpSession session) {
	...
	@RequestMapping(value = "/copyRowSetById")
	@ResponseBody
	public Object copyRowSetById(HttpServletRequest request,HttpServletResponse response
, HttpSession session) throws  Exception {
		// get dimension bean from request
		ItemRowSetHeaderBean headerBean=null;
		...
		if(!(Tan.lp().equals(Comp.dp()))){
			if(Loap.la(request).equals(ThirdLevelType.HQ)){
		try {
			String rowSetId = this.getDecodedRequestParam(request, "rowSetId");
			int id=Integer.parseInt(rowSetId);
			headerBean=rptSettingService.getRowSetHeaderByByPrimaryKey(id);
			int flag=rptSettingService.copyRowSet(headerBean);
			map.put("flag", flag);
			list.add(map);
