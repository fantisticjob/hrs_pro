...
function addTabInParent(title,url){
	...
	var parentJq = top.jQuery;
	//先判断是否存在标题为title的选项卡
	var tab= parentJq('#tt').tabs('exists',title);
	
	if(tab){
		//若存在则直接打开
		parentJq('#tt').tabs('select',title);
	}else{
		//否则创建
		parentJq('#tt').tabs('add',{
			title:title,
...
function currentTabRefresh(){
	...
	var parentJq = top.jQuery;
	//先判断是否存在标题为title的选项卡
	var currTab= parentJq('#tt').tabs('getSelected');
	var url = $(currTab.panel('options').content).attr('src');	
	if (currTab) {
		parentJq('#tt').tabs('update', {
		    tab : currTab,
		    options : {
		        content : "<iframe width='100%' height='100%'  id='iframe' frameborder='0' scrolling='auto' src='"+url+"'></iframe>",
				closable:true
