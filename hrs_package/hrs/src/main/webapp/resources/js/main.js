...
//				$("#topmenu").empty();
//				$("#topmenu").append("&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"javascript:addTab('首页','../resources/html/welcome.htm')\" >首&nbsp;&nbsp;页</a>");
//				if(data.length > 0){
//					//循环加载第一级别菜单
...
//		$("#topmenu > a").get(0).click();	
		//切换一级菜单 加载二级和三级菜单
		//切换一级菜单 加载二级和三级菜单
		$("#topmenu").on("click", "a", function(){			//设置样式
			$("#topmenu > a").removeClass("active");
			$(this).addClass("active");
			//alert(this);
			//加载 二级菜单和三级菜单
			var menu1icode = $(this).attr("icode");
			//alert("menu1icode"+menu1icode);
			if(!JUDGE.isNull(menu1icode)){	
				$('#tt1').tree({
				    url:"findLgoinSecondMenu.do?maindatauuid="+menu1icode,
