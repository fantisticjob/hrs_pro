	...
	<script type="text/javascript">     
	    $(function(){
	    	...
	    	$('#tableview').edatagrid({
				...
				columns:[[
				    {field:'INSTANCE_ID', title:'任务ID', width:50},
					{field:'LEDGER_ID', title:'账套',width:50,
					...
					{field:'FIN_ELEMENT_TYPE',title:'财务要素',width:50,
						formatter:function(value, row, index){
					...
					{field:'PERIOD_FROM',title:'会计区间起',width:50,
						formatter:function(value, row, index){
					...
					{field:'PERIOD_TO',title:'会计区间至',width:50,
						formatter:function(value, row, index){
						  	for(var i=0; i < periodList.length; i++){
							    if (periodList[i].PERIODID == value) 
								    return periodList[i].PERIODNAME;
...
<body>
    <form:form id="requestExceForm" commandName="executCondition" action="" method="post">        
        <div id="tb" style="padding:3px">
	        <span>账套:</span>
	            <form:select path="ledgerId" id="ledgerId" name="ledgerId" items="${executCondition.ledgerSelectionList}" itemLabel="lederName" itemValue="lederCode" class="easyui-combobox" style="width:130"></form:select>
	        &nbsp;&nbsp;&nbsp;<span>财务要素类型:</span>
