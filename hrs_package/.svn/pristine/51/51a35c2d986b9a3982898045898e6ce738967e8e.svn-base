...
public class EtlProcessingController extends BaseController {
	...
	@ResponseBody
	public Object getEtlJobList (HttpServletRequest request,HttpServletResponse response, Model model) {
		// 参数
	...
	@RequestMapping(value = "/updateEtlJbo")
	@ResponseBody
	...
	public Object processEtlJob(HttpServletRequest request,HttpServletResponse response, HttpSession session) {
		...
		try {
			...
			if (StringUtils.isBlank(jobId)) {
				...
				if(scheduleId==null){
					try {
						list=etlJobService.processEtlJob(reqBean);
					} catch (Exception e) {
						logger.error("processEtlJob():", e);
					}	
				}else{
					try {
						list=etlJobService.processEtlSchedule(reqBean);
					} catch (Exception e) {
	...
	//XXX allocation job running configurations    GXY 2017-03-29
	/*  
	 *     add attribute jobType in JobInstanceBean ,sharing the common bean ,update DAO implements to update anther table 
	 *  
	 *  start  */
	...
	private JobInstanceBean constructJobBean(
		...
		if (request != null ) {
			...
			//set parameter list
			jobBean.setParameList(this.getDecodedRequestParam(request, "PARAM_LIST"));
			//set description
			jobBean.setDescription(this.getDecodedRequestParam(request, "DESCRIPTION"));
			
			jobBean.setConcurrent(this.getDecodedRequestParam(request, "CONCURRENT"));
			if (!isCreated) {
	...
	private void constructJobReturnMap(JobInstanceBean jobBean) {
		...
		if (jobBean != null) {
			...
			dataMap.put("PARAM_LIST", jobBean.getParameList());
