...
public class QuartzManager {  
    ...
    public static void addJob(String jobName, Class cls, String time,JobInstanceBean reqBean,IJobProcessDao jobProcessDao) { 
    	try {
            ...
            jobDetail.getJobDataMap().put("jobProcessDao",jobProcessDao);
     ...
     */  
    @SuppressWarnings("rawtypes")  
    public static void modifyJobTime(String jobName,Class cls, String time,JobInstanceBean reqBean,IJobProcessDao jobProcessDao) {  
        try {  
            Scheduler sched = gSchedulerFactory.getScheduler();
            TriggerKey triggerKey=new TriggerKey(jobName,TRIGGER_GROUP_NAME);
            ...
            if (!oldTime.equalsIgnoreCase(time)) {  
                removeJob(jobName);  
                addJob(jobName, cls, time,reqBean,jobProcessDao);  
            }  
        } catch (Exception e) {  
            throw new RuntimeException(e);  
        }  
    ...
    @SuppressWarnings("rawtypes")  
    public static void addJob(String jobName, Class cls, String time,AllocRequestInstanceHistoryRecord requestHistoryBean, IDataProcessDao dataProcessDao,ICalculationDao calculationDao) { 
    	try {
            Scheduler sched = gSchedulerFactory.getScheduler();  
            // 任务名，任务组，任务执行类  
            JobDetail jobDetail=JobBuilder.newJob(cls).withIdentity(jobName, JOB_GROUP_NAME).build();             //可以传递参数  
            jobDetail.getJobDataMap().put("requestHistoryBean",requestHistoryBean); 
            jobDetail.getJobDataMap().put("dataProcessDao",dataProcessDao);
            jobDetail.getJobDataMap().put("calculationDao",calculationDao);
            jobDetail.getJobDataMap().put("jobName",jobName);
            // 触发器  
            CronTrigger trigger = (CronTrigger)TriggerBuilder.newTrigger()  
            .withIdentity(jobName, TRIGGER_GROUP_NAME).withSchedule(CronScheduleBuilder.cronSchedule(time)).build() ;  
    ...
    public static void modifyJobTime(String jobName,Class cls, String time,AllocRequestInstanceHistoryRecord requestHistoryBean,
			IDataProcessDao dataProcessDao,ICalculationDao calculationDao) {  
        try {  
            Scheduler sched = gSchedulerFactory.getScheduler();
            TriggerKey triggerKey=new TriggerKey(jobName,TRIGGER_GROUP_NAME);
            CronTrigger trigger = (CronTrigger) sched.getTrigger(triggerKey);  
            ...
            if (!oldTime.equalsIgnoreCase(time)) {  
                removeJob(jobName);  
                addJob(jobName, cls, time,requestHistoryBean,dataProcessDao,calculationDao);  
            }  
        } catch (Exception e) {  
            throw new RuntimeException(e);  
        }  
